...
Use the personal Codex marketplace entry generated by the plugin creator, then install `office-os` from the ChatGPT desktop app. Current Codex builds load plugin skills and MCP servers, but lifecycle hooks must be registered in the user's config layer. From the installed Office OS root, register its three lifecycle hooks once:

```powershell
python scripts/office_hook_registry.py install --plugin-root (Resolve-Path .).Path
```

The registry writes only Office OS's `SessionStart`, `UserPromptSubmit`, and `Stop` groups to `~/.codex/hooks.json`. It gives them one fixed plugin root and plugin-data root, preserves unrelated user hook groups, and replaces rather than duplicates its own entries. To remove only those three entries later, run `python scripts/office_hook_registry.py uninstall`. Open `/hooks` once to review and trust the exact hook definitions; changed hooks require review again.
