...
| Todo 16 rollup validation remediation | Rollup reference parser and SIZE_OK marker proof | `powershell -NoProfile -ExecutionPolicy Bypass -File scripts\qa\final-scope-fidelity.ps1 ...`; temporary injected missing rollup reference; PowerShell parser checks; `git diff --check` | positive run exits 0 with 173 rollup references checked, negative injected missing reference exits 1, parser checks pass, oversized QA harnesses have in-file SIZE_OK markers | `task-16-rollup-reference-validation-final-scope.txt`, `task-16-rollup-reference-validation-negative-probe.txt`, `task-16-rollup-reference-validation-negative-run.txt`, `task-16-rollup-reference-validation-static-checks.txt` |
...
- Todo 16: `task-16-linear-comments.json`, `task-16-code-review.txt`, `task-16-manual-qa-matrix.md`, `task-16-notepad.md`, `task-16-linear-rollup.md`, `task-16-formatting.txt`, `task-16-remediation-formatting.txt`, `task-16-remediation-proof.txt`, `task-16-e2e-remediation-full.txt`, `task-16-e2e-assertion-remediation.txt`, `task-16-plugin-smoke-remediation.txt`, `task-16-static-remediation-checks.txt`, `task-16-final-scope-remediation.txt`, `task-16-remote-list-remediation-summary.txt`, `task-16-upstream-and-cleanliness-remediation.txt`, `task-16-gate-blocker-remediation.txt`, `task-16-rollup-blocker-remediation.txt`, `task-16-run-hermes-cli-behavior.txt`, `task-16-rollup-omission-final-scope-fidelity.txt`, `task-16-rollup-omission-final-scope-fidelity-post-push.txt`, `task-16-rollup-omission-remediation.txt`, `task-16-rollup-reference-validation-final-scope.txt`, `task-16-rollup-reference-validation-negative-probe.txt`, `task-16-rollup-reference-validation-negative-run.txt`, `task-16-rollup-reference-validation-static-checks.txt` - Linear governance/evidence handoff proof, review bundle, final scope-fidelity gate, remediation notes, focused formatting/syntax checks, full E2E/smoke/static/scope reruns, current-command stdout assertion proof, remote list remediation summary, upstream/cleanliness proof, final blocker remediation proof, rollup blocker remediation proof, Hermes CLI behavior proof, rollup omission remediation proof, rollup reference parser proof, negative missing-reference proof, and SIZE_OK marker/static proof. All listed Todo 16 remediation artifacts exist under the evidence root and were checked as non-empty during the Todo 16 rollup omission remediation.
