...
The hook owns one private intake marker keyed by separate hashes of session and session-plus-turn. It stores no raw prompt, caps live markers at 128, expires them after one hour on the next intake or Stop, and consumes the matching marker at Stop; a newer source-free turn in the same session replaces the older marker. Stop can consume the same-session marker when the host advances the turn id. This bounded control state is not Office data or workspace workflow state.
