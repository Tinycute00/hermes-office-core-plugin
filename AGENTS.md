...
Hooks and MCP processes receive the plugin-owned data root. The hook must inject that authoritative `PLUGIN_DATA` value into Office workflow context, and the skill must pass it unchanged to every core/manager child process; never rely on the agent terminal's ambient environment or a second fallback root. Hook and core state locking must remain interoperable through the same OS-owned file lock.
